    ...
    if (isset($_POST['rate_b'])) {
        try {
            ...
            if ($buyer_comment) {
                ...
                $product_id = filter_var($_GET['product-id'], FILTER_VALIDATE_INT);

                $stmt02 = $db->prepare('update ratings set buyer_rate=:rate ,comment=:comment,rate_date=:date where buyer_id=:id 
            AND item_id=:iid');
                $stmt02->bindParam(':rate', $rating, PDO::PARAM_INT);
                $stmt02->bindParam(':comment', $comment, PDO::PARAM_STR);
                $stmt02->bindParam(':date', $r_date, PDO::PARAM_STR);
                $stmt02->bindParam(':id', $user_id, PDO::PARAM_INT);
                $stmt02->bindParam(':iid', $product_id, PDO::PARAM_INT);
                $stmt02->execute();
            ...
            ');
                $stmt03->bindParam(':bid', $user_id, PDO::PARAM_INT);
                $stmt03->bindParam(':iid', $product_id, PDO::PARAM_INT);
                $stmt03->bindParam(':rate', $rating, PDO::PARAM_INT);
    ...
    <div class="comments">
        <?php foreach ($comments as $comment) { ?>
            <div class="comment-container">
                ...
                <div class="comment-header"><?php echo htmlspecialchars($comment['user_name'],ENT_QUOTES,'UTF-8'); ?></div>
                <div class="comment-date"><?php echo htmlspecialchars($comment['rate_date'],ENT_QUOTES,'UTF-8'); ?></div>
                <div class="comment-rating"><?php echo str_repeat('★', htmlspecialchars($comment['buyer_rate'],ENT_QUOTES,'UTF-8')) . str_repeat('☆', (5 - $comment['buyer_rate'])) ?></div>
